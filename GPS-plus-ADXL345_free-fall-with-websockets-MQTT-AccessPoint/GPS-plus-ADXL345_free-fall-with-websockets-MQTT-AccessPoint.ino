...
//gps setup
static const int RXPin = 14, TXPin = 12; // blue is T , yellow R
...
void setup ( void ) {
  ...
  config.ssid = "ssid";
  config.password = "password";
  config.mqttServer = "smartbox1.eastus2.cloudapp.azure.com" ;//"ec2-54-227-228-246.compute-1.amazonaws.com";

  Serial.print("Connecting to ");
  Serial.println(config.ssid.c_str()); Serial.println(config.password.c_str());
...
void displayInfo()
{
  ...
  if (gps.location.isValid())
  {
    ...
    double distance = gps.distanceBetween(prevLat, prevLong, gps.location.lat(), gps.location.lng() );

    Serial.print(F(" - "));
    Serial.print(distance, 6);

    //if (prevLat != gps.location.lat() || prevLong != gps.location.lng())
    if (distance > 0.5 )
    {
      double alt = -1;
      ...
      if (gps.speed.isValid())
        speeed = gps.speed.mph();

      sprintf(gps_info, "{\"gps_location\":\"%s,%s,%s,%s\"}", String(gps.location.lat(), 6).c_str(), String(gps.location.lng(), 6).c_str(), String(alt, 6).c_str() ,  String(speeed, 6).c_str());
      postToServer(gps_info);
    }
  ...
  else
  {
    ...
    sprintf(gps_info, "{\"gps_location\":\"%d,%d\"}", -1, -1);
    postToServer(gps_info);
