...
const int ADMIN_MODE_PIN = 16;//13 for the larger devices , 13 for the smaless one
...
void setup ( void ) {
  EEPROM.begin(1024);
  Serial.begin(115200);
  pinMode(ADMIN_MODE_PIN, INPUT_PULLDOWN_16);
  //digitalWrite(ADMIN_MODE_PIN, LOW);
  delay(500);
...
void setup_adxl()
{
  ...
  int m = millis();
  String t_no("{\"trackingNo\": \"1Z 999 AA1 01 2345 " + String(m) + "\"}");
  postToServer((char*)t_no.c_str());
}
...
void postToServer(const char* data)
{
  if (!client.connected()) {
    reconnect();
  }
